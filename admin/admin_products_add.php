...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($description) || empty($price) || empty($stock) || empty($category_id)) {
        $message = "Semua kolom wajib diisi (kecuali Rating dan Gambar).";
        $message_type = "error";
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($file_ext, $allowed_ext)) {
                ...
                if (!move_uploaded_file($file_tmp_name, $upload_dir . $image_url)) {
        ...
        if (empty($message)) {
            try {
                $sql = "INSERT INTO products (name, description, price, stock, category_id, image_url, rating)
                        VALUES (?, ?, ?, ?, ?, ?, ?)";
                $stmt = $pdo->prepare($sql);
                $stmt->execute([$name, $description, $price, $stock, $category_id, $image_url, $rating]);

                header("Location: admin_products.php?message=Produk+berhasil+ditambahkan&type=success");
                exit();

            } catch (PDOException $e) {
                error_log("Error adding product: " . $e->getMessage());
                $message = "Terjadi kesalahan saat menambahkan produk: " . $e->getMessage();
                $message_type = "error";
            }
        }
    }
...
<head>
    ...
    <title>Tambah Produk Baru - Admin Soncake</title>
    ...
    <main class="main-content">
        <div class="product-form-container">
            ...
            <form action="" method="post" enctype="multipart/form-data">
                ...
                <input type="number" id="stock" name="stock" min="0" required value="<?php echo htmlspecialchars($_POST['stock'] ?? ''); ?>">
                ...
                <label for="image">Gambar Produk (Max 5MB, JPG/PNG/GIF):</label>
                <input type="file" id="image" name="image" accept="image/jpeg, image/png, image/gif">

                <label for="rating">Rating (0-5, opsional):</label>
                <input type="number" id="rating" name="rating" step="0.1" min="0" max="5" value="<?php echo htmlspecialchars($_POST['rating'] ?? '0.0'); ?>">
